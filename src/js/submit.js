...
if(stepIndex === steps.length - 1){
  ...
  if (typeof window.checkRateLimit === 'function') {
    const rateCheck = window.checkRateLimit('form_submission', 3, 3600000); // 3 per hour
    if (!rateCheck.allowed) {
      showAlert(`Please wait ${rateCheck.waitMinutes} minutes before submitting again. This helps prevent spam.`, "warn");
      return;
    }
  }
  ...
  if (typeof window.sanitizeText === 'function') {
    Object.keys(payload).forEach(key => {
      if (typeof payload[key] === 'string' && key !== 'website_url' && key !== 'general_email' && key !== 'admin_contact_email' && key !== 'intake_email') {
        payload[key] = window.sanitizeText(payload[key], 5000);
      }
    });
  }
  
  // Validate URLs
  if (payload.website_url && typeof window.validateUrl === 'function' && !window.validateUrl(payload.website_url)) {
    showAlert("Invalid website URL. Please enter a valid http:// or https:// URL.", "warn");
    return;
  }
  
  // Validate emails
  if (payload.general_email && typeof window.validateEmail === 'function' && !window.validateEmail(payload.general_email)) {
    showAlert("Invalid email address format.", "warn");
    return;
  }
  
  if (payload.admin_contact_email && typeof window.validateEmail === 'function' && !window.validateEmail(payload.admin_contact_email)) {
    showAlert("Invalid admin contact email format.", "warn");
    return;
  }
  ...
  if (typeof window.logSecurityEvent === 'function') {
    window.logSecurityEvent('form_submission_attempt', { org: payload.org_name, program: payload.program_name });
  }
  ...
  try{
    ...
    if (typeof window.logSecurityEvent === 'function') {
      window.logSecurityEvent('form_submission_success', { org: payload.org_name });
    }
  }catch(err){
    if (typeof window.logSecurityEvent === 'function') {
      window.logSecurityEvent('form_submission_failed', { error: err.message });
    }
    if(String(err && err.message) === "FORM_ENDPOINT_NOT_CONFIGURED"){
      showAlert("Admin setup required: paste your Formspree endpoint into FORMSPREE_ENDPOINT in js/submit.js.", "warn");
    } else {
