...
**Data-ops scripts** (see `npm run` names in `package.json`): `validate-data` (schema/integrity, blocking), `audit-program-data` / `spot-check-25` (crawl + audit; `audit-program-data:dry` for report-only), `data:patch` → `scripts/apply-patches.js` (apply a reviewed batch of record corrections), `verify-program-links` / `apply-link-fixes` (link health), `sync-regional-data` (regions dataset), `assign-waves` (rolling re-verification slots), `geocode-programs` (lat/lng), `remove-program`. The four `apply-*-2026*.js` / `apply-spot-check-fixes.js` scripts are frozen one-off batches kept for provenance — write new batches as patchfiles for `apply-patches.js`, not as new scripts. `verify-all-programs.js` and `mark-programs-verified.js` were retired: the first had dead skip logic (a hardcoded `REVIEW_DATE`) and reported bot-blocked sites as healthy, the second stamped every program verified regardless of outcome.
...
- **Catch-up.** `--wave N` also sweeps anything ≥80 days old regardless of wave (`CATCH_UP_DAYS`, `audit-program-data.js:438`), so a skipped or failed run cannot let a program quietly expire.
- **A failed fetch is not a verdict.** The crawler retries transient failures (network error, timeout, 5xx, 429) once with jittered backoff, then consults `scripts/state/url-health.json` — a **committed** failure history, because the audit reports are gitignored and the rule needs memory between runs. A URL is only called down after failing in **two consecutive runs**; a first failure *holds* the program instead: prior `verification.status` and prior `last_verified` are both kept, and `verification.hold_reason` + `last_attempted_at` record why. Entries are deleted when a URL recovers and pruned when it stops being referenced.
- **403/429 are refusals, not verifications.** They used to be coerced to `ok`, so a site that blocks bots read as a healthy verification. They now set `bot_blocked`, hold the prior status the same way, and route the program to the manual queue. Anything held is listed under `held_programs` in `scripts/data-audit-summary.json`.
