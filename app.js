...
// Ensure fallbacks exist if storage.js fails to load
if (typeof window.loadEncryptedData !== 'function') {
  window.loadEncryptedData = async function(key, defaultValue = []) {
    return JSON.parse(localStorage.getItem(key) || JSON.stringify(defaultValue));
  };
}

if (typeof window.saveEncryptedData !== 'function') {
  window.saveEncryptedData = async function(key, data) {
    localStorage.setItem(key, JSON.stringify(data));
  };
}

const loadEncryptedDataSafe = window.loadEncryptedData;
const saveEncryptedDataSafe = window.saveEncryptedData;
...
async function initializeEncryptedStorage() {
  favorites = new Set(await loadEncryptedDataSafe('favorites', []));
  recentSearches = await loadEncryptedDataSafe('recentSearches', []);
  callHistory = await loadEncryptedDataSafe('callHistory', []);
  customLists = await loadEncryptedDataSafe('customLists', {});
  programNotes = await loadEncryptedDataSafe('programNotes', {});
  programTags = await loadEncryptedDataSafe('programTags', {});
  const prefs = await loadEncryptedDataSafe('userPreferences', {});
...
async function trackCallAttempt(program) {
  ...
  await saveEncryptedDataSafe('callHistory', callHistory);
...
async function saveFavorites() {
  await saveEncryptedDataSafe('favorites', Array.from(favorites));
  updateFavoritesCount();
}
...
async function saveProgramNote(programId, note) {
  ...
  await saveEncryptedDataSafe('programNotes', programNotes);
}
...
async function addProgramTag(programId, tag) {
  ...
  if (!programTags[programId].includes(sanitizedTag) && sanitizedTag.trim()) {
    ...
    await saveEncryptedDataSafe('programTags', programTags);
  }
}
...
async function createCustomList(listName) {
  ...
  if (sanitizedName.trim() && !customLists[sanitizedName]) {
    ...
    await saveEncryptedDataSafe('customLists', customLists);
...
async function addRecentSearch(query) {
  ...
  await saveEncryptedDataSafe('recentSearches', recentSearches);
